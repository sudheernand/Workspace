...
public class EmployeeDAO implements IEmployeeDAO {
	static final Logger loggerObj = Logger.getLogger(EmployeeDAO.class);

	public Employee select(Integer id) throws PersistException {
		DOMConfigurator.configure("property.xml");
	...
	public boolean create(Employee empDetailsObj) throws PersistException {
		...
		if (empDetailsObj.getFirstName() == null) {
			throw new PersistException("employee cant be null");
		}
		if (empDetailsObj.getLastName() == null) {
			throw new PersistException("lastname cant be null");
		}
